...
# class InterfuserAgent(MapAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        case_name = '087fec73-1a0c-399a-9292-cc2cf99dc97f' # pedestrain
        ...
        image_list = []
        image_dir = f'{argo_path}/{case_name}/cameras/ring_front_center'
        ...
        self.rgb_front =[]
        for data in rgb:
            self.rgb_front.append(data[:1800, :])

        image_list = []
        image_dir = f'{argo_path}/{case_name}/cameras/ring_front_left'
        file_list = os.listdir(image_dir)
        file_list.sort()
        for image_name in file_list:
            image_list.append(cv2.cvtColor(cv2.imread(os.path.join(image_dir, image_name)), cv2.COLOR_BGR2RGB))
        ...
        self.rgb_left =[]
        for data in rgb:
            self.rgb_left.append(data)

        image_list = []
        image_dir = f'{argo_path}/{case_name}/cameras/ring_front_right'
        file_list = os.listdir(image_dir)
        file_list.sort()
        for image_name in file_list:
            image_list.append(cv2.cvtColor(cv2.imread(os.path.join(image_dir, image_name)), cv2.COLOR_BGR2RGB))
        ...
        self.rgb_right =[]
        for data in rgb:
            self.rgb_right.append(data)

        image_list = []
        image_dir = f'{argo_path}/{case_name}/cameras/ring_rear_left'
        file_list = os.listdir(image_dir)
        file_list.sort()
        for image_name in file_list:
            image_list.append(cv2.cvtColor(cv2.imread(os.path.join(image_dir, image_name)), cv2.COLOR_BGR2RGB))
        ...
        self.rgb_rear_left =[]
        for data in rgb:
            self.rgb_rear_left.append(data)

        self.target_waypoint = [np.ones(shape=(2)) * np.array([0.1, -30.0])] * 320
        ## for intersection left turn ##
        # self.target_waypoint = np.array(self.target_waypoint)
        x_value = -20 + 20/170*np.arange(1, 171) #15
        self.target_waypoint[150:] = [np.array([x_value[i], -7.0]) for i in range(170)] # -10
        # self.target_waypoint[159:][:, 0] = -10 + 10 / np.arange(1, 162)[::-1] 
        # self.target_waypoint[159:][:, 1] = -3.0
        # self.target_waypoint = [self.target_waypoint][0]
        ...
        self.net.load_state_dict(torch.load(path_to_model_file, map_location=torch.device('cpu'))["model"])
    ...
    def run_step(self, input_data, timestamp):
        ...
        self.rgb_front_current = self.rgb_front.pop()
        self.rgb_left_current = self.rgb_left.pop()
        self.rgb_right_current = self.rgb_right.pop()
        self.rgb_rear_left_current = self.rgb_rear_left.pop()
        ...
        if self.config.mode == 'onboard':
            ...
            detection_states = torch.tensor(self.detection_states, dtype=torch.bfloat16).unsqueeze(0).to(device=self.device)
            actions = torch.tensor(self.actions, dtype=torch.bfloat16)
            actions = torch.cat([actions, torch.zeros((1, self.config.act_dim), dtype=torch.bfloat16)], dim=0).unsqueeze(0).to(device=self.device)
            target_waypoints = torch.tensor(self.target_waypoints, dtype=torch.bfloat16).unsqueeze(0).to(device=self.device)
            rtgs = torch.tensor(self.rtgs, dtype=torch.bfloat16).unsqueeze(0).to(device=self.device)
            timesteps = torch.tensor(self.timesteps, dtype=torch.int32).unsqueeze(0).to(device=self.device)
        else:
            img_states = torch.tensor(self.img_states, dtype=torch.float32).to(device=self.device)
            img_states = img_states / 255.
            detection_states = torch.tensor(self.detection_states, dtype=torch.float32).unsqueeze(0).to(device=self.device)
            ...
            target_waypoints = torch.tensor(self.target_waypoints, dtype=torch.float32).unsqueeze(0).to(device=self.device)
            rtgs = torch.tensor(self.rtgs, dtype=torch.float32).unsqueeze(0).to(device=self.device)
            timesteps = torch.tensor(self.timesteps, dtype=torch.int32).unsqueeze(0).to(device=self.device)
        ...
        if len(self.rgb_front) < 140 and len(self.rgb_front) > 130:
            detection_states[:,:,0] = 1.0
        ...
        # else:
        with torch.no_grad():
            actions, actions_one_hot, trajectory = self.net.get_action(img_states, actions, rtgs, timesteps, detection_states,
        ...
        tick_data["rgb_left"] = cv2.resize(self.rgb_left_current, (200, 150))
        tick_data["rgb_right"] = cv2.resize(self.rgb_right_current, (200, 150))
        tick_data["rgb_focus"] = cv2.resize(self.rgb_rear_left_current, (150, 150))
        ...
        tick_data["language"] = "Instruction:" + self.net.prompt_gpt.vlm_adapter.prompt[-1]
