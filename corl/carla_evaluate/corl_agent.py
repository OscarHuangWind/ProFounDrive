...
class DisplayInterface(object):
    ...
    def run_interface(self, input_data):
        ...
        surface = cv2.putText(surface, input_data['control'], (20,560), cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,255), 1)
        surface = cv2.putText(surface, input_data['speed'], (20,540), cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,0,255), 1)
...
# class InterfuserAgent(MapAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        self.logit_route_list = []
        self.action_speed_list = []
        self.logit_speed_list = []
        ...
        self.control_point = 2 #2 for town04
        self.control_margin = 0.5
    ...
    def tick(self, input_data):
        ...
        local_target_point = np.array([target_wp[0] - pos[0], target_wp[1] - pos[1]])
        local_target_point = R.T.dot(local_target_point)
        result["target_local_waypoint"] = local_target_point

        local_next_point = np.array([next_wp[0] - pos[0], next_wp[1] - pos[1]])
        local_next_point = R.T.dot(local_next_point)
        result["local_next_waypoint"] = local_next_point

        center_point = [-self._world.get_map().get_waypoint(self._vehicle.get_location()).transform.location.y,
                        self._world.get_map().get_waypoint(self._vehicle.get_location()).transform.location.x]
        local_center_point = np.array([center_point[0] - pos[0], center_point[1] - pos[1]])
        local_center_point = R.T.dot(local_center_point)
        result["local_center_point"] = local_center_point
    ...
    def run_step(self, input_data, timestamp):
        ...
        detection = tick_data['detection'] # array of float64 (5,)
        decision = None
        target_waypoint, target_command = tick_data['target_local_waypoint'], None #array of float64 (2,)

        rgb = cv2.resize(tick_data["rgb"], tuple(self.config.img_resolution)) # array of uint8 (224, 224, 3)
        
        if self.config.image_type == 'segmentation':
            segmentation_image = self.net.seg_encoder.forward(rgb) # array of uint8 (224, 224, 8)
        ...
        self_car_map = render_self_car(
            ...
            color=[1, 1, 0], pixels_per_meter=10, max_distance=20,
        )

        # trajectory = np.concatenate((trajectory, trajectory_refined), axis=0)
        trajectory = np.concatenate((trajectory, target_waypoint[np.newaxis,:]), axis=0)
        ...
        if self.config.model == 'GPT':
            tick_data["language"] = "Instruction:" + self.net.prompt_gpt.vlm_adapter.prompt_list[-1]
        else:
        ...
        self.action_route_list.append(int(next_route_command-1))
        self.action_speed_list.append(int(next_speed_command))
        self.logit_route_list.append(route_logit.tolist())
    ...
    def save_json(self, data, measurements_file):
        f = open(measurements_file, "w")
        json.dump(data, f, indent=4)
        f.close()

    def save_csv(self, data):
        result_file = "./decision_baseline_new_pedestrian.csv"
    ...
    def _is_light_red(self, lights_list):
        ...
        light = self._find_closest_valid_traffic_light(
            self._vehicle.get_location(), min_dis=5 #8
        )
    ...
    @torch.no_grad()
    def _smooth_trajectory(self, tick_data, trajectory, next_node, next_route_command):
        junction = self._world.get_map().get_waypoint(self._vehicle.get_location()).is_junction
        keep_lane_decision = (LAT_DECISION_LIST[next_route_command - 1] == 'Keep Lane')
        trajectory_refined = trajectory.copy()
        if not junction and keep_lane_decision and abs(tick_data['local_center_point'][0] - next_node[0]) > self.control_margin:
            trajectory_refined[abs(tick_data['local_center_point'][0] - trajectory[:,0]) > self.control_margin, 0] = (tick_data['local_center_point'][0] + trajectory_refined[abs(tick_data['local_center_point'][0] - trajectory[:,0])> self.control_margin, 0])/2
            self._turn_controller._K_P = 1.25
        else:
