...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  4 10:43:43 2023
...
class CodaPrompt(nn.Module):
    def __init__(self, emb_d, n_tasks, prompt_param, key_dim=768, device='cuda:0'):
        ...
        for e in self.e_layers:
            ...
            e_l = self.e_p_length
            p = tensor_prompt(self.e_pool_size, e_l, emb_d, device=device)
...
class DualPrompt(nn.Module):
    def __init__(self, emb_d, n_tasks, prompt_param, key_dim=768, device='cuda:0'):
        ...
        for g in self.g_layers:
            p = tensor_prompt(self.g_p_length, emb_d, device=device)
            setattr(self, f'g_p_{g}',p)

        # e prompt init
        for e in self.e_layers:
            p = tensor_prompt(self.e_pool_size, self.e_p_length, emb_d, device=device)
            k = tensor_prompt(self.e_pool_size, self.key_d, device=device)
            setattr(self, f'e_p_{e}',p)
            setattr(self, f'e_k_{e}',k)
