...
def displacement_error(pred, target):
    ...
    mask = np.not_equal(target[:, :, :2], 0)

    for i in range(len(pred)):
        if mask[i, 0, 0]:
            error = np.linalg.norm(pred[i, :, :2] - target[i, :, :2], axis=-1) 
...
def miss_rate(pred, target):
    pred = pred.detach().cpu().numpy()
    target = target.detach().cpu().numpy()
    distance_to_ref = T.distance.cdist(pred[:, :2], target[:, :2])
    distance_to_route = np.min(distance_to_ref, axis=-1)

    miss_count = (distance_to_route > 0.3).sum()
    miss_rate = miss_count / len(pred)
