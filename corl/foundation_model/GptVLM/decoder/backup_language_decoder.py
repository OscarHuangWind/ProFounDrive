...
class PromptGPT(nn.Module):
    ...
    def __init__(
        ...
        if self.config.mode == 'onboard':
            self.llm_model = llm_model.from_pretrained('gpt2-xl', torch_dtype=torch.bfloat16)
            self.embed_timestep = nn.Embedding(max_emb_size, embed_size, dtype=torch.bfloat16)
            ...
            self.embed_return = torch.nn.Linear(1, embed_size, dtype=torch.bfloat16)
            self.embed_action = torch.nn.Linear(self.act_dim, embed_size, dtype=torch.bfloat16) # individual embedding for action to prevent info leakage
            ...
            self.embed_timestep = nn.Embedding(max_emb_size, embed_size, dtype=torch.float32)
            self.embed_image_state = torch.nn.Linear(self.config.encoder_embed_dim, embed_size, dtype=torch.float32)
            self.embed_return = torch.nn.Linear(1, embed_size, dtype=torch.float32)
            self.embed_action = torch.nn.Linear(self.act_dim, embed_size, dtype=torch.float32) # individual embedding for action to prevent info leakage
    ...
    def forward(self, states, actions, returns_to_go, timesteps,
        ...
        action_embeddings = self.project_action(action_embeddings)
        rtg_embeddings = self.project_rtg(rtg_embeddings)

        # if self.config.mode == 'onboard':
        #     token_embeddings = torch.zeros((batch_size, seq_length, 
        #                                     image_state_embeddings.shape[2] +\
        #                                     rtg_embeddings.shape[2] +\
        #                                     action_embeddings.shape[2],
        #                                     self.embed_size), dtype=torch.bfloat16,
        #                                     device=image_state_embeddings.device)
        # else:
        #     token_embeddings = torch.zeros((batch_size, seq_length, 
        #                                     image_state_embeddings.shape[2] +\
        #                                     rtg_embeddings.shape[2] +\
        #                                     action_embeddings.shape[2],
        #                                     self.embed_size), dtype=torch.float32,
        #                                     device=image_state_embeddings.device)

        # this makes the sequence look like (Rtg_1, S_image1_1,..., S_imageN_1, a_1, R_2, S_image1_2,..., S_imageN_2, a_2, ...)
        # token_embeddings = torch.cat(
        #     (rtg_embeddings, image_state_embeddings, action_embeddings), dim=2
        # ).permute(0, 2, 1, 3).reshape(batch_size, seq_length, -1, self.embed_size)
        ...
        out, multimodal_attention_mask = self.vlm_adapter(input_dicts, get_embeddings_func)
        
        llm_embeddings = torch.cat((rtg_embeddings, out, action_embeddings), dim=-2)
