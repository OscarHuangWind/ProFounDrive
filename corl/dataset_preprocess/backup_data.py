...
# from corl.foundation_model.vision_encoder.segformer import SegEncoder
...
class CARLA_Data(Dataset):
    ...
    def __init__(self, root, config, shared_dict=None):
        ...
        for sub_root in tqdm(root, file=sys.stdout):
            ...
            for route in routes:
                ...
                for seq in range(1, num_seq): # skip first and last frame
                    skip_flag = False
                    ...
                    # bev_i = np.array(route_dir / "birdview" / ("%04d.jpg" % (seq))).astype(np.string_)
                    ...
                    # bev_array = cv2.imread(str(bev_i, encoding='utf-8'), cv2.IMREAD_UNCHANGED)
                    # bev_array = cv2.cvtColor(bev_array, cv2.COLOR_BGR2RGB)
                    # if (bev_array is None):
                    #     print("Error loading file: ", str(bev_i, encoding='utf-8'))
                    # bev_array = np.moveaxis(bev_array, -1, 0)
                    # bev_i = decode_pil_to_npy(bev_array).astype(np.uint8)
                    
                    # image and bev augmentation
                    crop_shift = 0
                    degree = 0
                    ...
                    # bev_i = load_crop_bev_npy(bev_i, degree)
                    
                    # self.step_reward_list.append(step_reward)
                    # image.append(image_i)
                    # bev.append(bev_i)
                    # measurement.append(measurement_i)
                    # step_reward.append(step_reward)
                    ...
                    next_x = measurement_i['near_node_x']
                    next_y = measurement_i['near_node_y']
                    trajectory = measurement_i['future_waypoints'][1:self.pred_len+1]
                    next_gt = np.array([next_x, next_y])
                    ...
                    next_waypoint = np.array([next_x-ego_x, next_y-ego_y])
                    next_waypoint = R.T.dot(next_waypoint)
                    ...
                    for i in range(len(trajectory)):
                        next_wp = trajectory[i][:2]
                        next_local_wp = R.T.dot(next_wp - np.array((ego_x, ego_y)))
                        local_trajectory.append(next_local_wp)

                    self.images.append(image_i)
                    # self.bevs.append(bev_i)
                    self.measurements.append(measurement_i)
                    self.steers.append(measurement_i['steer'])
                    self.throttles.append(measurement_i['throttle'])
                    self.brakes.append(measurement_i['brake'])
                    # self.global_decisions.append(measurement_i['gt_command'])
                    # self.local_decisions.append(measurement_i['command'])
                    self.step_rewards.append(measurement_i['reward'])
                    # self.target_velocities.append(measurement_i['target_speed'])
                    self.detections.append(detection)
                    self.local_decisions.append(local_decision)
                    self.next_waypoints.append(next_waypoint)
                    self.next_gts.append(next_gt)
                    self.target_waypoints.append(target_waypoint)
                    self.trajectory.append(np.array(local_trajectory))
                ...
                seudo_local_decisions = np.ones(shape=(3, len(self.local_decisions[0]))) * self.local_decisions[0]
                seudo_detections = np.ones(shape=(3, len(self.detections[0]))) * self.detections[0]
                seudo_next_waypoints = np.ones(shape=(3, len(self.next_waypoints[0]))) * self.next_waypoints[0]
                seudo_next_gts = np.ones(shape=(3, len(self.next_gts[0]))) * self.next_gts[0]      
                seudo_target_waypoints = np.ones(shape=(3, len(self.target_waypoints[0]))) * self.target_waypoints[0]
                seudo_masks = np.zeros(shape=(3))
                seudo_trajectory = np.ones(shape=(len(self.trajectory[0]), len(self.trajectory[0][0]))) * self.trajectory[0]
                # for i in range(2, -1, -1):
                #     seudo_masks = np.zeros(shape=(1,self.seq_len))
                #     seudo_masks[0,-(i+1):] += 1
                #     self.masks = np.insert(self.masks, 0, seudo_masks, axis=0)    
                
                self.images = np.insert(self.images, 0, seudo_image, axis=0).tolist()
                self.step_rewards = np.insert(self.step_rewards, 0, seudo_step_rewards, axis=0).tolist()
                self.throttles = np.insert(self.throttles, 0, seudo_throttles, axis=0).tolist()
                self.brakes = np.insert(self.brakes, 0, seudo_brakes, axis=0).tolist()
                self.steers = np.insert(self.steers, 0, seudo_steers, axis=0).tolist()
                self.local_decisions = np.insert(self.local_decisions, 0, seudo_local_decisions, axis=0).tolist()
                self.detections = np.insert(self.detections, 0, seudo_detections, axis=0).tolist()
                self.next_waypoints = np.insert(self.next_waypoints, 0, seudo_next_waypoints, axis=0).tolist()
                self.next_gts = np.insert(self.next_gts, 0, seudo_next_gts, axis=0).tolist()
                self.target_waypoints = np.insert(self.target_waypoints, 0, seudo_target_waypoints, axis=0).tolist()
                self.masks = np.insert(self.masks, 0, seudo_masks, axis=0).tolist()
                for i in range(3):
                    self.trajectory.insert(0, seudo_trajectory)
                self.done_idxs.append(num_seq - 1 - skip_counter + 3) # skip first frame
                # self.done_idxs.append(num_seq - skip_counter)
            ...
            # seudo_timesteps = np.zeros(shape=(4))
            # self.timesteps = np.insert(self.timesteps, 0, seudo_timesteps, axis=0)

        
        self.images = np.array(self.images)
        # self.bevs = np.array(self.bevs)
        self.step_rewards = np.array(self.step_rewards)
        self.throttles = np.array(self.throttles)
        self.brakes = np.array(self.brakes)
        self.steers = np.array(self.steers)
        # self.global_decisions = np.array(self.global_decisions)
        self.local_decisions = np.array(self.local_decisions)
        self.timesteps = np.array(self.timesteps)
        self.detections = np.array(self.detections)
        self.next_waypoints = np.array(self.next_waypoints)
        self.next_gts = np.array(self.next_gts)
        self.trajectory = np.array(self.trajectory)
        # self.target_velocities = np.array(self.target_velocities)
        self.done_idxs = np.array(self.done_idxs)
    ...
    def __getitem__(self, index):
        ...
        decisions = torch.tensor(self.local_decisions[idx:done_idx], dtype=torch.float32)
        next_gts = torch.tensor(self.next_gts[idx:done_idx], dtype=torch.float32)
        ...
        # timesteps = torch.tensor(self.timesteps[idx:idx+1], dtype=torch.int32).unsqueeze(1)

        throttles = self.throttles[idx:done_idx]
        brakes = self.brakes[idx:done_idx]
        steers = self.steers[idx:done_idx]

        # done_idx = index + self.pred_len
        ...
        #         done_idx = min(int(cum_idx), done_idx) #redefine the done_idx
        #         break
        # idx = done_idx - self.pred_len

        trajectory = torch.tensor(self.trajectory[done_idx])    
        traj_len = trajectory.shape[0]
        trajectory = torch.cat([trajectory, torch.ones((self.pred_len - traj_len, trajectory.shape[1])) * trajectory[-1]], dim=0)

        # target_velocities = torch.tensor(self.target_velocities[idx:done_idx], dtype=torch.float64).unsqueeze(1)
        # velo_len = target_velocities.shape[0]
        # target_velocities = torch.cat([target_velocities, torch.ones((self.pred_len - velo_len, target_velocities.shape[1])) * target_velocities[-1]], dim=0)    

        next_waypoints = torch.tensor(self.next_waypoints[idx:done_idx])
        wp_len = next_waypoints.shape[0]
        next_waypoints = torch.cat([next_waypoints, torch.ones((self.pred_len - wp_len, next_waypoints.shape[1])) * next_waypoints[-1]], dim=0)
        ...
        # print(actions)
        # # Waypoints: target points
        # # convert x_command, y_command to local coordinates
        # # taken from LBC code (uses 90+theta instead of theta)
        # ego_theta = measurements[self.seq_len-1]['theta'] # + rad for augmentation
        # ego_x = measurements[self.seq_len-1]['gps_x']
        # ego_y = measurements[self.seq_len-1]['gps_y']
        ...
        # data['local_goal_waypoint'] = local_command_point
        
        # # data['target_point_image'] = draw_target_point(local_command_point)
        
        # command_waypoints = []
        # for i in range(min(10, len(measurements[self.seq_len-1]["future_waypoints"]))):
        #     waypoint = measurements[self.seq_len-1]["future_waypoints"][i]
        #     new_loc = R.T.dot(np.array([waypoint[0] - ego_x, waypoint[1] - ego_y]))
        #     # new_loc = R.T.dot(np.array([waypoint[1] - ego_y, waypoint[0] - ego_x]))
        ...
        data['next_waypoints'] = next_waypoints
        data['next_gts'] = next_gts
        data['target_waypoints'] = target_waypoints
        # data['target_velocities'] = target_velocities
