...
class AutoPilot(MapAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        # self.daytimes_ids = list(self.daytimes)
        
        # self.index = random.choice(range(len(self.weathers)))
        # self.weather_id = self.weathers_ids[self.index] + self.daytimes_ids[self.index]
    ...
    def _init(self):
        ...
        ########################################################################
        # weather = self.weathers[self.weathers_ids[self.index]]
        # weather.sun_altitude_angle = self.daytimes[self.daytimes_ids[self.index]]
        # weather.sun_azimuth_angle = 0.0 #np.random.choice(self.azimuths)
        ...
        self._max_speed = 6.0 # m/s

        # self.control_step = -1
        # data_path = '/media/oscar/Samsung_T5/CarlaData/InterFuser'
        # setting = 'debug-town'
        # config = GlobalConfig(root_dir=data_path, setting=setting)
        # self.train_dataset_root = config.val_data
        # town = 'town07'
        # train_town_dataset_root = []
        # for j in self.train_dataset_root:
        #     if town in j:
        #         train_town_dataset_root.append(j)
        # self.carla_train_dataset = CARLA_Data(root=train_town_dataset_root, config=config, shared_dict=None)
    ...
    def run_step(self, input_data, timestamp):
        ...
        # action reward
        if abs(steer - self._last_steer) > 0.01:
            r_action = -0.1
        else:
            r_action = 0.0
        ...
        r_speed = 1.0 - np.abs(ego_speed - target_speed) / self._max_speed

        # displacement reward
        ego_theta = data["compass"]
        pos = self._get_position(data)
        ego_x = pos[0]
        ego_y = pos[1]
        ...
        #       'ego_y:', ego_y, '\n',
        #       'ego_theta:', ego_theta, '\n',
        #       'ego_waypoint:', ego_waypoint_local, '\n',
        #       'r_speed:', r_speed, '\n',
        #       'r_action:', r_action, '\n',
        #       'r_pos:', r_position, '\n',
        #       'reward:', reward)
        ...
        ###############################################################
        # self.control_step += 1
        # data = self.carla_train_dataset[self.control_step]
        # print(self.control_step, self.step)
